            ...
            foreach (var modFile in modsDirectory)
            {
                ...
                try
                {
                    ...
                    if (jar.GetEntry("META-INF/mods.toml") is ZipArchiveEntry forgeEntry)
                    {
                        ...
                        Console.WriteLine($"[{scannedFileCount()}/{totalFileCount}] [SCANNER] [INFO] Forge mod scanned: {fileName}");
                    }
                    ...
                    else if (jar.GetEntry("fabric.mod.json") is ZipArchiveEntry fabricEntry)
                    {
                        ...
                        Console.WriteLine($"[{scannedFileCount()}/{totalFileCount}] [SCANNER] [INFO] Fabric mod scanned: {fileName}");
                    }
                }
                catch
                {
                    failedScanCount ++;
                    Console.WriteLine($"[{scannedFileCount()}/{totalFileCount}] [SCANNER] [ERROR-001] failed to scan file: {fileName}");
                }
            }

            Console.WriteLine($"[SCANNER] [INFO] {fabricModCount + forgeModCount} mods scanned out of {totalFileCount} files in folder: {failedScanCount} failed scan(s)");
        }
